...
| `--log path.jsonl` | 两者 | `logs/events.jsonl` | JSONL 日志路径。默认文件只记录拦截到的原始请求/响应；显式传 `--log` 路径时记录完整诊断事件。可用 `--log false` 关闭。 |
| `--login-timeout-ms 300000` | 两者 | `300000` | 手动登录轮询超时时间。 |
| `--timeout-ms 60000` | 两者 | `60000` | 页面和操作超时时间。 |
| `--queue true|false` | 服务 | `true` | `serve` 模式下让 `/api/trends` 创建 SQLite 查询任务并返回 id。设为 `false` 可用于本地同步调试。 |
| `--queue-db path.sqlite` | 服务 | `data/queue.sqlite` | `serve` 模式下 SQLite 队列数据库路径。 |
| `--baidu-rate-limit true|false` | 百度 | `true` | 是否启用百度频率控制。设为 `false` 会跳过最小间隔和冷却等待。 |
| `--baidu-min-interval-ms 15000` | 百度 | `15000` | 同一 profile 下百度查询之间的最小间隔。传 `0` 可关闭。 |
| `--baidu-cooldown-ms 120000` | 百度 | `120000` | 百度返回频率风控后的冷却时间。传 `0` 可关闭。 |
...
| `--area 0` | 百度 | `0` | 百度地区代码。 |
| `--baidu-mode page|api` | 百度 | `page` | 百度采集模式。`page` 直接导航到趋势页并拦截页面发出的搜索/资讯指数 XHR；page 失败会直接暴露。`api` 在已认证上下文里直接调内部接口。 |
| `--google-mode page|api` | Google | `page` | Google 采集模式。`page` 打开 explore 页面并拦截 UI 自己发的响应；page 失败或返回空数据会直接暴露。`api` 直接调内部 Trends 接口（更快但 headless 时容易被静默限流）。 |
...
| `OHMYTRENDS_QUEUE` | 设为 `false` 可让 `serve` 模式下直接 `/api/trends` 请求绕过队列。 |
| `OHMYTRENDS_QUEUE_DB` | `serve` 模式下 SQLite 队列数据库路径。 |
| `OHMYTRENDS_BAIDU_RATE_LIMIT` | 设为 `false` 可关闭百度频率控制。 |
...
`/api/trends` 请求默认会创建 SQLite 持久化查询任务，并立即返回查询 id。
客户端用 `GET /api/trends/<query-id>` 轮询状态和结果。同一 source/profile 的
重复请求会串行执行，并复用百度频率控制。如果希望 HTTP 请求等待排队任务完成，
在同一个 `/api/trends` 请求上加 `wait=true` 即可。也可以用 `--queue false`
或 `OHMYTRENDS_QUEUE=false` 关闭队列，让 `/api/trends` 在本地调试时同步返回最终结果。
队列任务默认持久化到 `data/queue.sqlite`。可用 `--queue-db path/to/queue.sqlite`
或 `OHMYTRENDS_QUEUE_DB` 覆盖。
...
任务成功后响应状态为 `succeeded`，`result` 字段里是和 `get --format json`
相同的统一 JSON 结构。失败任务会返回 `status: "failed"` 和 `error` 消息，
轮询路由的 HTTP 状态码为 500。响应数据契约请查看 [`json-output.zh-CN.md`](json-output.zh-CN.md)。

如果明确需要同步阻塞到采集完成，可以在同一个接口上加 `wait=true`；
这种方式仍然会经过队列和频率控制。也可以加 `waitTimeoutMs` 限制 HTTP
请求最多等待多久；到达上限时，API 会返回当前任务状态和 `pollUrl`，HTTP
状态码为 202，客户端可以继续轮询。
...
队列基于 SQLite，服务重启后仍会保留任务。后台 worker 会复用直接采集时的百度频率控制。
...
`loginTimeoutMs`。还支持 `baiduRateLimit`、`baiduMinIntervalMs`、
`baiduCooldownMs`、`baiduMode`、`googleMode`、`log`、`wait`、`waitTimeoutMs`
和 `lang`，其中 `lang` 可传 `en` 或 `zh` 控制运行时提示语言。
...
最低。Page 模式不会回退到 `api`；需要对比接口直连路径时，显式使用
`--google-mode api`。
...
URL 更不容易触发反爬。Page 模式不会回退到 `api`；需要测试接口直连路径时，
显式使用 `--baidu-mode api`。
